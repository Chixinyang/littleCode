...
def my_Cookieyar():
    ...
    #打印cookie信息
    print("cookie type is :",type(cookie))
    for item in cookie:
        print('Name = %s' % item.name)
        print('Value = %s' % item.value)

#2)将Cookie保存到文件中
def my_MozillaCookiejar():
    from urllib import request
    from http import cookiejar
    #设置保存cookie的文件，同级目录下的cookie.txt
    #filename = r'C:\Users\Public\Desktop\cookie.txt'
    filename = 'cookie.txt' #默认是当前文件所在目录下

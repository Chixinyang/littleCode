...
async def select(sql, args, size=None): #定义select
    log(sql, args)
    global __pool
    async with __pool.acquire() as conn: #获取一个连接
        ...
        return rs   #返回查询结果
import pdb
async def execute(sql, args, autocommit=True): #定义增删改的执行函数，共性是返回受影响的行数
#   pdb.set_trace()
    log(sql)
    async with __pool.acquire() as conn:
        ...
        try:
            ...
            if not autocommit:
                await conn.rollback()  #出现异常就回滚
            raise
        return affected

def create_args_string(num):#根据num创建对应数目的参数
    L = []
    for n in range(num): 
        L.append('?')
    return ', '.join(L)
